    ...
    if fromVer < 11:
        ...
        def replaceNames(d):
            if "position" in d['properties']:
                pos = d['properties'].pop("position")
                s = d['properties']["size"]
                d['properties']["center"] = [int(pos[0] + s[0]/2), int(pos[1] + s[1]/2)]
            if 'childModels' in d:
                for child in d['childModels']:
                    replaceNames(child)
        for c in dataDict['cards']:
    ...
    if fromVer < 11:
        ...
        def replaceNames(obj):
            for k ,v in obj.handlers.items():
                if len(v):
                    val = v
                    val = re.sub(r"\.position\b", ".center", val)
                    val = re.sub(r"\.animate_position\(\b", ".animate_center(", val)
                    obj.handlers[k] = val
            for child in obj.childModels:
                replaceNames(child)
        replaceNames(stackModel)
